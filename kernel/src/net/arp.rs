...
impl ArpEntry {
    const fn empty() -> Self {
        ArpEntry { ip: [0;4], mac: [0;6], age: 0 }
    }
    fn is_valid(&self) -> bool { self.ip != [0,0,0,0] }
}
...
impl ArpCache {
    ...
    fn insert(&mut self, ip: [u8; 4], mac: [u8; 6]) {
        ...
        for e in self.entries.iter_mut() {
            if !e.is_valid() {
                *e = ArpEntry { ip, mac, age: crate::arch::x86_64::interrupts::uptime_ms() };
                return;
            }
        }
        // Evict oldest
        ...
        for (i, e) in self.entries.iter().enumerate() {
            if e.age < oldest_age { oldest_age = e.age; oldest = i; }
        }
        self.entries[oldest] = ArpEntry { ip, mac, age: crate::arch::x86_64::interrupts::uptime_ms() };
    }
...
pub fn handle_packet(payload: &[u8]) {
    if payload.len() < ARP_PKT_LEN { return; }

    // Validate hardware and protocol type
    if u16::from_be_bytes([payload[0], payload[1]]) != 0x0001 { return; } // Ethernet
    if u16::from_be_bytes([payload[2], payload[3]]) != 0x0800 { return; } // IPv4
    if payload[4] != 6 || payload[5] != 4 { return; }
...
/// Send an ARP reply.
fn send_reply(our_mac: [u8; 6], our_ip: [u8; 4],
...
fn build_packet(op: u16,
                sender_mac: [u8; 6], sender_ip: [u8; 4],
                target_mac: [u8; 6], target_ip: [u8; 4]) -> [u8; ARP_PKT_LEN] {
    let mut p = [0u8; ARP_PKT_LEN];
    p[0] = 0x00; p[1] = 0x01;           // hw type = Ethernet
    p[2] = 0x08; p[3] = 0x00;           // proto type = IPv4
    p[4] = 6;    p[5] = 4;              // hw/proto addr lengths
    p[6] = (op >> 8) as u8;
    p[7] = (op & 0xFF) as u8;
