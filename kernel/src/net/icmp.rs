...
use spin::Mutex;
use crate::net::{eth, ipv4, config, arp};
...
impl ReplyQueue {
    const fn new() -> Self {
        ReplyQueue {
            entries: [PingReply { from: [0;4], id: 0, seq: 0, rtt_ms: 0 }; REPLY_BUF],
...
pub fn send_echo_request_to(dst_ip: [u8; 4], dst_mac: [u8; 6], id: u16, seq: u16) -> bool {
    ...
    if !crate::net::driver::is_ready() { return false; }
    ...
    for b in icmp[8..].iter_mut() { *b = 0xAB; }
    ...
    ipv4::build_header(&mut ip_hdr, ip_id, 64, PROTO_ICMP, cfg.ip, dst_ip, icmp.len());
    ...
    LAST_SEND_MS.store(crate::arch::x86_64::interrupts::uptime_ms() as u32, Ordering::Relaxed);
    ...
    if ok { PINGS_SENT.fetch_add(1, Ordering::Relaxed); }
    ok
}
...
pub fn handle_packet(src_ip: [u8; 4], payload: &[u8]) {
    if payload.len() < ICMP_HDR { return; }
    ...
    match icmp_type {
        ICMP_ECHO_REPLY => {
            ...
            REPLY_QUEUE.lock().push(PingReply { from: src_ip, id, seq, rtt_ms: rtt });
        }
        ICMP_ECHO_REQUEST => {
...
fn send_echo_reply(dst_ip: [u8; 4], id: u16, seq: u16, data: &[u8]) {
    ...
    icmp[1] = 0;
    icmp[2] = 0; icmp[3] = 0;
    icmp[4] = (id >> 8) as u8; icmp[5] = (id & 0xFF) as u8;
    icmp[6] = (seq >> 8) as u8; icmp[7] = (seq & 0xFF) as u8;
    ...
    let ip_id = IP_ID.fetch_add(1, Ordering::Relaxed);
    ipv4::build_header(&mut ip_hdr, ip_id, 64, PROTO_ICMP, cfg.ip, dst_ip, icmp_total);
...
pub fn stats() -> (u32, u32) {
    (PINGS_SENT.load(Ordering::Relaxed), PINGS_RECV.load(Ordering::Relaxed))
}
