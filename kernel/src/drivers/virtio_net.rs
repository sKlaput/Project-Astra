...
use core::sync::atomic::{fence, AtomicBool, AtomicU16, AtomicU64, Ordering};
use spin::Mutex;
...
fn kernel_virt_to_phys(virt: usize) -> usize {
    let (phys_base, virt_base) = crate::boot::protocol::kernel_address().unwrap_or((0, 0));
...
pub fn init() -> bool {
    ...
    unsafe {
        ...
        out8(
            io_base + VIO_DEV_STATUS,
            STS_ACK | STS_DRIVER | STS_DRIVER_OK,
        );
        fence(Ordering::SeqCst);
...
pub fn send_frame(data: &[u8]) -> bool {
    ...
    if data.is_empty() || data.len() > 1514 {
        return false;
    }
