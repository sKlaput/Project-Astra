...
const GRAVITY_MS: [u64; 21] = [
    800, 716, 633, 550, 466, 383, 300, 216, 133, 100,
     83,  83,  83,  66,  66,  66,  50,  50,  50,  33, 33,
];
...
enum Phase { Playing, Paused, GameOver }
...
impl TetrisState {
    ...
    fn next_piece(&mut self) -> u8 {
        if self.bag_pos >= 7 { self.shuffle_bag(); }
    ...
    fn peek_next(&self) -> u8 {
        if self.bag_pos < 7 { self.bag[self.bag_pos] }
        else { self.bag[0] }
    }
    ...
    fn valid(&self, p: u8, r: usize, col: i8, row: i8) -> bool {
        for (c, rr) in self.cells(p, r, col, row) {
            if c < 0 || c >= COLS as i8 { return false; }
            if rr >= ROWS as i8 { return false; }
            if rr >= 0 && self.board[rr as usize][c as usize] != 0 { return false; }
        }
        true
    }
    ...
    fn ghost_row(&self) -> i8 {
        let mut r = self.pr;
        while self.valid(self.piece, self.rot, self.pc, r + 1) { r += 1; }
        r
    }
...
impl TetrisApp {
    pub fn new() -> Self {
        TetrisApp { inner: UnsafeCell::new(TetrisState::new()) }
    }
...
fn draw_cell(cx: usize, cy: usize, col_idx: u8, ghost: bool) {
    if col_idx == 0 { return; }
    let bg  = if ghost { GHOST_COL } else { PIECE_COLS[col_idx as usize] };
    let dk  = if ghost { GHOST_COL } else { PIECE_DARK[col_idx as usize] };
...
impl App for TetrisApp {
    fn title(&self) -> &str { "Tetris" }
    fn preferred_size(&self) -> (usize, usize) { (WIN_W, WIN_H) }
    fn app_id(&self) -> &'static str { "tetris" }
    ...
    fn render(&self, cx: usize, cy: usize, cw: usize, ch: usize) {
        ...
        // Board border
        framebuffer::fill_rect(bx.saturating_sub(2), by.saturating_sub(2), BOARD_W + 4, 2, BORDER);
        framebuffer::fill_rect(bx.saturating_sub(2), by + BOARD_H, BOARD_W + 4, 2, BORDER);
        framebuffer::fill_rect(bx.saturating_sub(2), by.saturating_sub(2), 2, BOARD_H + 4, BORDER);
        ...
        if s.phase == Phase::Playing || s.phase == Phase::Paused {
            for (c, r) in s.current_cells() {
                if c >= 0 && r >= 0 && (c as usize) < COLS && (r as usize) < ROWS {
                    draw_cell(bx + c as usize * CELL, by + r as usize * CELL, s.piece, false);
                }
            }
        }
        ...
        // Score
        draw_label(px + 2, py, "SCORE"); py += 12;
        draw_value_u32(px + 2, py, s.score); py += 18;

        // Lines
        draw_label(px + 2, py, "LINES"); py += 12;
        draw_value_u32(px + 2, py, s.lines); py += 18;

        // Level
        draw_label(px + 2, py, "LEVEL"); py += 12;
        draw_value_u32(px + 2, py, s.level as u32); py += 24;
    ...
    fn handle_key(&mut self, key: Key) -> AppAction {
        ...
        match key {
            Key::ArrowLeft  => { s.try_move(-1, 0); }
            Key::ArrowRight => { s.try_move( 1, 0); }
            Key::ArrowDown  => {
                if s.try_move(0, 1) {
                    s.score += 1;
                    s.last_drop_ms = uptime_ms();
            ...
            Key::ArrowUp    => { s.try_rotate(true); }
            Key::Char(b'z') | Key::Char(b'Z') => { s.try_rotate(false); }
            Key::Char(b' ') => { s.hard_drop(); }
            Key::Char(b'c') | Key::Char(b'C') => { s.do_hold(); }
