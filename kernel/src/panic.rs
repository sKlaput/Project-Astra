...
pub fn handle(info: &PanicInfo<'_>) -> ! {
    ...
    crate::framebuffer::draw_text_scaled(40, 100, "The system has encountered a fatal error.", 0xFFCCCC, 2);
    ...
    if let Some(location) = info.location() {
        ...
        for &b in location.file().as_bytes() {
            if pos >= line_buf.len() - 12 { break; }
            line_buf[pos] = b; pos += 1;
        }
        if pos < line_buf.len() { line_buf[pos] = b':'; pos += 1; }
        let mut n = location.line();
        if n == 0 {
            if pos < line_buf.len() { line_buf[pos] = b'0'; pos += 1; }
        } else {
            let mut tmp = [0u8; 10]; let mut ti = 0;
            while n > 0 && ti < tmp.len() { tmp[ti] = b'0' + (n % 10) as u8; ti += 1; n /= 10; }
            tmp[..ti].reverse();
            for &b in &tmp[..ti] { if pos < line_buf.len() { line_buf[pos] = b; pos += 1; } }
        }
