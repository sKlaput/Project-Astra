...
fn try_mount_at(part_start_lba: u64) -> bool {
    ...
    let total_secs = if total_secs32 != 0 { total_secs32 } else { total_secs16 };
...
pub fn disk_space_kb() -> (u64, u64) {
    if !is_mounted() { return (0, 0); }
    let part_start  = PART_START.load(Ordering::Relaxed);
    let rsvd        = RSVD_SECS.load(Ordering::Relaxed) as u64;
    let fat_size    = FAT_SIZE.load(Ordering::Relaxed) as u64;
    let spc         = SEC_PER_CLUS.load(Ordering::Relaxed) as u64;
    if fat_size == 0 || spc == 0 { return (0, 0); }
    ...
    for sec_off in 0..fat_size {
        ...
        let mut buf = [0u8; 512];
        if read_sector(lba, &mut buf).is_err() { continue; }
        for i in 0..128usize {
...
impl DirEntry {
    pub const EMPTY: Self = DirEntry {
        name: [0u8; 64], name_len: 0, is_dir: false, size: 0, cluster: 0,
    };
...
fn parse_dirent(e: &[u8]) -> Option<DirEntry> {
    ...
    let first = e[0];
    if first == 0x00 { return None; } // end of directory
    if first == 0xE5 { return None; } // deleted
    let attr = e[11];
    if attr == 0x0F { return None; } // LFN entry — handled in list_dir
    if attr & 0x08 != 0 { return None; } // volume label

    // Skip . and ..
    if e[0] == b'.' { return None; }
    ...
    while base_end > 0 && e[base_end - 1] == b' ' { base_end -= 1; }
    for i in 0..base_end {
        let c = e[i];
    ...
    while ext_end > 0 && e[8 + ext_end - 1] == b' ' { ext_end -= 1; }
    if ext_end > 0 && !is_dir {
        name[nlen] = b'.';
        nlen += 1;
        for i in 0..ext_end {
    ...
    Some(DirEntry { name, name_len: nlen, is_dir, size, cluster })
}
    ...
    F: FnMut(DirEntry) -> bool,
{
    ...
    loop {
        ...
        for sec_off in 0..spc {
            ...
            let mut buf = [0u8; 512];
            if read_sector(lba, &mut buf).is_err() { return; }
            ...
            for entry_off in (0..512usize).step_by(32) {
                ...
                if attr == 0x0F {
                    ...
                    lfn_len = 0;
                    for i in 0..64 { if lfn_buf[i] == 0 { break; } lfn_len = i + 1; }
                    continue;
                }
...
pub fn read_file(start_cluster: u32, file_size: u32, buf: &mut [u8]) -> usize {
    ...
    'outer: loop {
        ...
        for sec_off in 0..spc {
            if total >= max_read { break 'outer; }
            let lba = cluster_to_lba(cluster) + sec_off as u64;
            let mut sec = [0u8; 512];
            if read_sector(lba, &mut sec).is_err() { break 'outer; }
...
pub fn find_in_dir(dir_cluster: u32, name: &[u8]) -> Option<DirEntry> {
    ...
    list_dir(dir_cluster, |de| {
        ...
        if de.name_len == name.len() {
            ...
            if eq { result = Some(de); return false; }
        }
        true
    });
    result
...
fn fat_write_entry(cluster: u32, value: u32) {
    ...
    let mut buf = [0u8; 512];
    if read_sector(lba0, &mut buf).is_err() { return; }
...
fn alloc_cluster() -> Option<u32> {
    ...
    for sec_off in 0..fat_size {
        ...
        for i in 0..128usize {
            ...
            if cluster < 2 { continue; }
            if cluster >= max_cluster { return None; }
            let off = i * 4;
            let entry = read_le32(&buf, off) & 0x0FFF_FFFF;
            if entry == 0 {
...
fn make_83_name(name: &[u8]) -> [u8; 11] {
    ...
    if needs_lfn {
        ...
        for &b in base {
            ...
            if is_83_valid_char(ub) { out[n] = ub; n += 1; }
        }
...
fn is_83_compatible(name: &[u8]) -> bool {
    ...
    for &b in base.iter().chain(ext.iter()) {
        let ub = b.to_ascii_uppercase();
        if !is_83_valid_char(ub) { return false; }
    }
    true
}
...
fn fill_lfn_entry(buf: &mut [u8; 32], seq: usize, is_last: bool,
                   name: &[u8], name_len: usize, checksum: u8) {
    for b in buf.iter_mut() { *b = 0; }
    buf[0] = seq as u8 | if is_last { 0x40 } else { 0 };
    buf[11] = 0x0F; // LFN attribute
    buf[13] = checksum;
    buf[26] = 0; buf[27] = 0; // cluster (always 0 in LFN)
...
fn find_slot_run(
    ...
    'outer: loop {
        ...
        for sec_off in 0..spc as u64 {
            ...
            let mut buf = [0u8; 512];
            if read_sector(lba, &mut buf).is_err() { break 'outer; }
...
pub fn write_file(dir_cluster: u32, name: &[u8], data: &[u8]) -> bool {
    ...
    let lfn_n        = if needs_lfn { lfn_entry_count(name_len) } else { 0 };
    ...
    'scan: {
        ...
        loop {
            ...
            for sec_off in 0..spc as u64 {
                let lba = cluster_to_lba(cluster) + sec_off;
                let mut buf = [0u8; 512];
                if read_sector(lba, &mut buf).is_err() { break 'scan; }
                for eo in (0..512usize).step_by(32) {
                    let first = buf[eo];
                    if first == 0x00 { break 'scan; } // end of dir — not found
    ...
    let bytes_per_clus = spc * 512;
    let n_clusters = if data.is_empty() { 0 }
                     else { (data.len() + bytes_per_clus - 1) / bytes_per_clus };
    ...
    for _ in 0..n_clusters {
        let c = match alloc_cluster() { Some(c) => c, None => return false };
        if first_cluster == 0 { first_cluster = c; }
        if prev_cluster  != 0 { fat_write_entry(prev_cluster, c); }
        prev_cluster = c;
        for sec_off in 0..spc {
            ...
            if write_sector_raw(lba, &sec).is_err() { return false; }
        }
    }
    ...
    if found {
        ...
        let mut buf = [0u8; 512];
        if read_sector(dirent_lba, &mut buf).is_err() { return false; }
...
pub fn create_dir(parent_cluster: u32, name: &[u8]) -> bool {
    ...
    if find_in_dir(parent_cluster, name).is_some() { return false; }
    ...
    let new_c = match alloc_cluster() { Some(c) => c, None => return false };
    ...
    let parent_c = if parent_cluster == root_cluster() { 0u32 } else { parent_cluster };
    ...
    if write_sector_raw(first_lba, &dot_buf).is_err() { return false; }
    let zero = [0u8; 512];
...
pub fn delete_entry(dir_cluster: u32, name: &[u8]) -> bool {
    ...
    loop {
        ...
        for sec_off in 0..spc as u64 {
            ...
            for eo in (0..512usize).step_by(32) {
                let first = buf[eo];
                if first == 0x00 { return false; } // end of dir — not found
                if first == 0xE5 { continue; }     // already deleted
                let attr = buf[eo + 11];
                if attr == 0x0F || attr & 0x08 != 0 { continue; } // LFN / volume
                if buf[eo..eo + 11] != name_83 { continue; }
...
pub fn rename_entry(dir_cluster: u32, old_name: &[u8], new_name: &[u8]) -> bool {
    ...
    let lfn_n_new     = if needs_lfn_new { lfn_entry_count(name_len_new) } else { 0 };
    ...
    'find: {
        ...
        loop {
            ...
            for sec_off in 0..spc as u64 {
                let lba = cluster_to_lba(cluster) + sec_off;
                let mut buf = [0u8; 512];
                if read_sector(lba, &mut buf).is_err() { break 'find; }
                for eo in (0..512usize).step_by(32) {
                    let first = buf[eo];
                    if first == 0x00 { break 'find; }
                    if first == 0xE5 { lfn_win_len = 0; continue; }
                    let attr = buf[eo + 11];
                    if attr & 0x08 != 0 { lfn_win_len = 0; continue; } // volume label
                    if attr == 0x0F {
                    ...
                    // 8.3 entry
                    if buf[eo..eo + 11] != old_83 { lfn_win_len = 0; continue; }
                    // Found!
                    let hi = read_le16(&buf, eo + 20) as u32;
                    let lo = read_le16(&buf, eo + 26) as u32;
...
pub fn mkfs() -> bool {
    ...
    vbr[67] = 0x41; vbr[68] = 0x53; vbr[69] = 0x54; vbr[70] = 0x52;
