param(
    ...
    [string]$OutPrefix = "build/poste14-gui-envelope-guard-validate"
)
...
Write-Host "[poste14-gui-envelope-guard] focused gui envelope guardrails run"
& .\scripts\run-qemu.ps1 -Profile $BuildCfg -TimeoutSeconds $TimeoutSeconds -LogPath $logPath
$qemuExit = $LASTEXITCODE

if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-gui-envelope-guard] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "gui-envelope-guard: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 GUI Envelope Guardrails Validation: $result",
...
Write-Host "[poste14-gui-envelope-guard] summary -> $summaryPath"
Write-Host "[poste14-gui-envelope-guard] json -> $jsonPath"
