param(
    ...
    [string]$OutPrefix = "build/poste14-gui-recover2-validate"
)
...
Write-Host "[poste14-gui-recover2] focused gui cooldown recovery run"
& .\scripts\run-qemu.ps1 -Profile $BuildCfg -TimeoutSeconds $TimeoutSeconds -LogPath $logPath
$qemuExit = $LASTEXITCODE

if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-gui-recover2] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "gui-recover2: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 GUI Cooldown Recovery Validation: $result",
...
Write-Host "[poste14-gui-recover2] summary -> $summaryPath"
Write-Host "[poste14-gui-recover2] json -> $jsonPath"
