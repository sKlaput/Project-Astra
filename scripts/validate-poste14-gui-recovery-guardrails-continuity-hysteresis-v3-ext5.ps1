#!/usr/bin/env pwsh
# Focused validator for Post-E14 Slice 94: GUI recovery guardrails continuity hysteresis v3 extended baseline (fifth cycle)

param(
    [string]$OutPrefix = "build/poste14-guirecoverguardconthyst3ext5-s94"
)
...
$failHits += @($log | Where-Object {
    $_ -like "gui-recover-guard-cont-hyst3-ext5: *FAIL" -or
...
$summary = @()
$summary += "Post-E14 Slice 94: GUI recovery guardrails continuity hysteresis v3 extended baseline (fifth cycle)"
...
$jsonSummary = @{
    slice = 94
    name = "GUI recovery guardrails continuity hysteresis v3 extended baseline (fifth cycle)"
