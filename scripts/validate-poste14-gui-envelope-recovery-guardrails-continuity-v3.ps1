param(
    ...
    [string]$OutPrefix = "build/poste14-gui-envelope-recover-guard-cont3-validate"
)
...
Write-Host "[poste14-gui-envelope-recover-guard-cont3] focused gui envelope recovery guardrails continuity v3 run"
& .\scripts\run-qemu.ps1 -Profile $BuildCfg -TimeoutSeconds $TimeoutSeconds -LogPath $logPath
$qemuExit = $LASTEXITCODE

if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-gui-envelope-recover-guard-cont3] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "gui-envelope-recover-guard-cont3: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 GUI Envelope Recovery Guardrails Continuity v3 Validation: $result",
...
Write-Host "[poste14-gui-envelope-recover-guard-cont3] summary -> $summaryPath"
Write-Host "[poste14-gui-envelope-recover-guard-cont3] json -> $jsonPath"
