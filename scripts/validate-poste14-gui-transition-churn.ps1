param(
    ...
    [string]$OutPrefix = "build/poste14-gui-churn-validate"
)
...
Write-Host "[poste14-gui-churn] focused gui transition churn run"
& .\scripts\run-qemu.ps1 -Profile $BuildCfg -TimeoutSeconds $TimeoutSeconds -LogPath $logPath
$qemuExit = $LASTEXITCODE

if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-gui-churn] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "gui-churn: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 GUI Transition Churn Validation: $result",
...
Write-Host "[poste14-gui-churn] summary -> $summaryPath"
Write-Host "[poste14-gui-churn] json -> $jsonPath"
