param(
    ...
    [string]$OutPrefix = "build/poste14-gui-throttle-dur-validate"
)
...
Write-Host "[poste14-gui-throttle-dur] focused gui throttling durability run"
& .\scripts\run-qemu.ps1 -Profile $BuildCfg -TimeoutSeconds $TimeoutSeconds -LogPath $logPath
$qemuExit = $LASTEXITCODE

if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-gui-throttle-dur] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "gui-throttle-dur: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 GUI Throttling Durability Validation: $result",
...
Write-Host "[poste14-gui-throttle-dur] summary -> $summaryPath"
Write-Host "[poste14-gui-throttle-dur] json -> $jsonPath"
