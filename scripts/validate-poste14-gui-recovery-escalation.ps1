param(
    ...
    [string]$OutPrefix = "build/poste14-gui-escalate-validate"
)
...
Write-Host "[poste14-gui-escalate] focused gui recovery escalation run"
& .\scripts\run-qemu.ps1 -Profile $BuildCfg -TimeoutSeconds $TimeoutSeconds -LogPath $logPath
$qemuExit = $LASTEXITCODE

if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-gui-escalate] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "gui-escalate: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 GUI Recovery Escalation Validation: $result",
...
Write-Host "[poste14-gui-escalate] summary -> $summaryPath"
Write-Host "[poste14-gui-escalate] json -> $jsonPath"
