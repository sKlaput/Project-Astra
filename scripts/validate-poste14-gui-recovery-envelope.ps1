param(
    ...
    [string]$OutPrefix = "build/poste14-gui-recover-envelope-validate"
)
...
Write-Host "[poste14-gui-recover-envelope] focused gui recovery envelope run"
& .\scripts\run-qemu.ps1 -Profile $BuildCfg -TimeoutSeconds $TimeoutSeconds -LogPath $logPath
$qemuExit = $LASTEXITCODE

if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-gui-recover-envelope] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "gui-recover-envelope: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 GUI Recovery Envelope Validation: $result",
...
Write-Host "[poste14-gui-recover-envelope] summary -> $summaryPath"
Write-Host "[poste14-gui-recover-envelope] json -> $jsonPath"
