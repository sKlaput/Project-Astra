param(
    ...
    [string]$OutPrefix = "build/poste14-gui-guard-hyst-recover-validate"
)
...
Write-Host "[poste14-gui-guard-hyst-recover] focused gui guardrails hysteresis recovery run"
& .\scripts\run-qemu.ps1 -Profile $BuildCfg -TimeoutSeconds $TimeoutSeconds -LogPath $logPath
$qemuExit = $LASTEXITCODE

if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-gui-guard-hyst-recover] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "gui-guard-hyst-recover: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 GUI Guardrails Hysteresis Recovery Validation: $result",
...
Write-Host "[poste14-gui-guard-hyst-recover] summary -> $summaryPath"
Write-Host "[poste14-gui-guard-hyst-recover] json -> $jsonPath"
