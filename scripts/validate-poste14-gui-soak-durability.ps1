param(
    ...
    [string]$OutPrefix = "build/poste14-gui-soak-dur-validate"
)
...
Write-Host "[poste14-gui-soak-dur] focused gui soak durability run"
& .\scripts\run-qemu.ps1 -Profile $BuildCfg -TimeoutSeconds $TimeoutSeconds -LogPath $logPath
$qemuExit = $LASTEXITCODE

if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-gui-soak-dur] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "gui-soak-dur: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 GUI Soak Durability Validation: $result",
...
Write-Host "[poste14-gui-soak-dur] summary -> $summaryPath"
Write-Host "[poste14-gui-soak-dur] json -> $jsonPath"
