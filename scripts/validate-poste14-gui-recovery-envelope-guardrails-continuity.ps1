param(
    ...
    [string]$OutPrefix = "build/poste14-gui-recover-envelope-guard-cont-validate"
)
...
Write-Host "[poste14-gui-recover-envelope-guard-cont] focused gui recovery envelope guardrails continuity run"
& .\scripts\run-qemu.ps1 -Profile $BuildCfg -TimeoutSeconds $TimeoutSeconds -LogPath $logPath
$qemuExit = $LASTEXITCODE

if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-gui-recover-envelope-guard-cont] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "gui-recover-envelope-guard-cont: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 GUI Recovery Envelope Guardrails Continuity Validation: $result",
...
Write-Host "[poste14-gui-recover-envelope-guard-cont] summary -> $summaryPath"
Write-Host "[poste14-gui-recover-envelope-guard-cont] json -> $jsonPath"
