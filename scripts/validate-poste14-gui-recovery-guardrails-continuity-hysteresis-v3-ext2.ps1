#!/usr/bin/env pwsh
# Focused validator for Post-E14 Slice 79: GUI recovery guardrails continuity hysteresis v3 extended baseline (second cycle)

param(
    [string]$OutPrefix = "build/poste14-guirecoverguardconthyst3ext2-s79"
)
...
$failPatterns = @(
    "gui-recover-guard-cont-hyst3-ext2: .*FAIL",
    "page fault",
    "panic"
)
...
$summary = @()
$summary += "Post-E14 Slice 79: GUI recovery guardrails continuity hysteresis v3 extended baseline (second cycle)"
...
$jsonSummary = @{
    slice = 79
    name = "GUI recovery guardrails continuity hysteresis v3 extended baseline (second cycle)"
