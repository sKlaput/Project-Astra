param(
    ...
    [string]$OutPrefix = "build/poste14-gui-input-validate"
)
...
Write-Host "[poste14-gui-input] focused gui input routing run"
& .\scripts\run-qemu.ps1 -Profile $BuildCfg -TimeoutSeconds $TimeoutSeconds -LogPath $logPath
$qemuExit = $LASTEXITCODE

if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-gui-input] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "gui-input: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 GUI Input Routing Validation: $result",
...
Write-Host "[poste14-gui-input] summary -> $summaryPath"
Write-Host "[poste14-gui-input] json -> $jsonPath"
