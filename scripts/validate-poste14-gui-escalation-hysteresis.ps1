param(
    ...
    [string]$OutPrefix = "build/poste14-gui-hysteresis-validate"
)
...
Write-Host "[poste14-gui-hysteresis] focused gui escalation hysteresis run"
& .\scripts\run-qemu.ps1 -Profile $BuildCfg -TimeoutSeconds $TimeoutSeconds -LogPath $logPath
$qemuExit = $LASTEXITCODE

if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-gui-hysteresis] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "gui-hysteresis: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 GUI Escalation Hysteresis Validation: $result",
...
Write-Host "[poste14-gui-hysteresis] summary -> $summaryPath"
Write-Host "[poste14-gui-hysteresis] json -> $jsonPath"
