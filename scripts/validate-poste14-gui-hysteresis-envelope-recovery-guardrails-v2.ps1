param(
    ...
    [string]$OutPrefix = "build/poste14-gui-hyst-envelope-recover-guard2-validate"
)
...
Write-Host "[poste14-gui-hyst-envelope-recover-guard2] focused gui hysteresis envelope recovery guardrails v2 run"
& .\scripts\run-qemu.ps1 -Profile $BuildCfg -TimeoutSeconds $TimeoutSeconds -LogPath $logPath
$qemuExit = $LASTEXITCODE

if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-gui-hyst-envelope-recover-guard2] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "gui-hyst-envelope-recover-guard2: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 GUI Hysteresis Envelope Recovery Guardrails v2 Validation: $result",
...
Write-Host "[poste14-gui-hyst-envelope-recover-guard2] summary -> $summaryPath"
Write-Host "[poste14-gui-hyst-envelope-recover-guard2] json -> $jsonPath"
