param(
    ...
    [string]$OutPrefix = "build/poste14-gui-focus-validate"
)
...
Write-Host "[poste14-gui-focus] focused gui focus arbitration run"
& .\scripts\run-qemu.ps1 -Profile $BuildCfg -TimeoutSeconds $TimeoutSeconds -LogPath $logPath
$qemuExit = $LASTEXITCODE

if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-gui-focus] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "gui-focus: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 GUI Focus Validation: $result",
...
Write-Host "[poste14-gui-focus] summary -> $summaryPath"
Write-Host "[poste14-gui-focus] json -> $jsonPath"
