param(
    ...
    [string]$OutPrefix = "build/poste14-apic-validate"
)
...
if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-apic] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "apic: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 APIC Validation: $result",
...
Write-Host "[poste14-apic] summary -> $summaryPath"
Write-Host "[poste14-apic] json -> $jsonPath"
