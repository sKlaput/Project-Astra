#!/usr/bin/env pwsh
# Focused validator for Post-E14 Slice 93: GUI envelope recovery guardrails continuity v3 extended baseline (fourth cycle)

param(
    [string]$OutPrefix = "build/poste14-guienveloperecoverguardcont3ext4-s93"
)
...
$failHits += @($log | Where-Object {
    $_ -like "gui-envelope-recover-guard-cont3-ext4: *FAIL" -or
...
$summary = @()
$summary += "Post-E14 Slice 93: GUI envelope recovery guardrails continuity v3 extended baseline (fourth cycle)"
...
$jsonSummary = @{
    slice = 93
    name = "GUI envelope recovery guardrails continuity v3 extended baseline (fourth cycle)"
