param(
    ...
    [string]$OutPrefix = "build/poste14-gui-recover-dur-validate"
)
...
Write-Host "[poste14-gui-recover-dur] focused gui recovery durability run"
& .\scripts\run-qemu.ps1 -Profile $BuildCfg -TimeoutSeconds $TimeoutSeconds -LogPath $logPath
$qemuExit = $LASTEXITCODE

if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-gui-recover-dur] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "gui-recover-dur: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 GUI Recovery Durability Validation: $result",
...
Write-Host "[poste14-gui-recover-dur] summary -> $summaryPath"
Write-Host "[poste14-gui-recover-dur] json -> $jsonPath"
