#!/usr/bin/env pwsh
# Focused validator for Post-E14 Slice 90: GUI guardrails continuity hysteresis envelope v3 extended baseline (fourth cycle)

param(
    [string]$OutPrefix = "build/poste14-guiguardconthystenvelope3ext4-s90"
)
...
$failHits += @($log | Where-Object {
    $_ -like "gui-guard-cont-hyst-envelope3-ext4: *FAIL" -or
...
$summary = @()
$summary += "Post-E14 Slice 90: GUI guardrails continuity hysteresis envelope v3 extended baseline (fourth cycle)"
...
$jsonSummary = @{
    slice = 90
    name = "GUI guardrails continuity hysteresis envelope v3 extended baseline (fourth cycle)"
