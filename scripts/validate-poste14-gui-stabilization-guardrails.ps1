param(
    ...
    [string]$OutPrefix = "build/poste14-gui-stabilize-guard-validate"
)
...
Write-Host "[poste14-gui-stabilize-guard] focused gui stabilization guardrails run"
& .\scripts\run-qemu.ps1 -Profile $BuildCfg -TimeoutSeconds $TimeoutSeconds -LogPath $logPath
$qemuExit = $LASTEXITCODE

if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-gui-stabilize-guard] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "gui-stabilize-guard: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 GUI Stabilization Guardrails Validation: $result",
...
Write-Host "[poste14-gui-stabilize-guard] summary -> $summaryPath"
Write-Host "[poste14-gui-stabilize-guard] json -> $jsonPath"
