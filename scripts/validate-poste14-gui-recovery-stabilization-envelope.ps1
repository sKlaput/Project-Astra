param(
    ...
    [string]$OutPrefix = "build/poste14-gui-recover-stabilize-envelope-validate"
)
...
Write-Host "[poste14-gui-recover-stabilize-envelope] focused gui recovery stabilization envelope run"
& .\scripts\run-qemu.ps1 -Profile $BuildCfg -TimeoutSeconds $TimeoutSeconds -LogPath $logPath
$qemuExit = $LASTEXITCODE

if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-gui-recover-stabilize-envelope] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "gui-recover-stabilize-envelope: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 GUI Recovery Stabilization Envelope Validation: $result",
...
Write-Host "[poste14-gui-recover-stabilize-envelope] summary -> $summaryPath"
Write-Host "[poste14-gui-recover-stabilize-envelope] json -> $jsonPath"
