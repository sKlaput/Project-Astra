#!/usr/bin/env pwsh
# Focused validator for Post-E14 Slice 87: GUI hysteresis envelope recovery guardrails v3 extended baseline (third cycle)

param(
    [string]$OutPrefix = "build/poste14-guihystenveloperecoverguard3ext3-s87"
)
...
$failPatterns = @(
    "gui-hyst-envelope-recover-guard3-ext3: .*FAIL",
    "page fault",
    "panic"
)
foreach ($pattern in $failPatterns) {
    $patternHits = $log | Select-String -Pattern $pattern
    if ($null -ne $patternHits) { $failHits += @($patternHits) }
}

$passFlag = ($missingMarkers.Count -eq 0) -and ($failHits.Count -eq 0)

$summary = @()
$summary += "Post-E14 Slice 87: GUI hysteresis envelope recovery guardrails v3 extended baseline (third cycle)"
...
$jsonSummary = @{
    slice = 87
    name = "GUI hysteresis envelope recovery guardrails v3 extended baseline (third cycle)"
