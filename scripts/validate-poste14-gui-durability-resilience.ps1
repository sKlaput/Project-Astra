param(
    ...
    [string]$OutPrefix = "build/poste14-gui-resilience-validate"
)
...
Write-Host "[poste14-gui-resilience] focused gui durability resilience run"
& .\scripts\run-qemu.ps1 -Profile $BuildCfg -TimeoutSeconds $TimeoutSeconds -LogPath $logPath
$qemuExit = $LASTEXITCODE

if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-gui-resilience] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "gui-resilience: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 GUI Durability Resilience Validation: $result",
...
Write-Host "[poste14-gui-resilience] summary -> $summaryPath"
Write-Host "[poste14-gui-resilience] json -> $jsonPath"
