#!/usr/bin/env pwsh
# Focused validator for Post-E14 Slice 78: GUI envelope recovery guardrails continuity v3 extended baseline

param(
    [string]$OutPrefix = "build/poste14-guienveloperecoverguardcont3ext-s78"
)
...
$failPatterns = @(
    "gui-envelope-recover-guard-cont3-ext: .*FAIL",
    "page fault",
    "panic"
)
...
$summary = @()
$summary += "Post-E14 Slice 78: GUI envelope recovery guardrails continuity v3 extended baseline"
...
$jsonSummary = @{
    slice = 78
    name = "GUI envelope recovery guardrails continuity v3 extended baseline"
