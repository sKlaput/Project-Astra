param(
    ...
    [string]$OutPrefix = "build/poste14-gui-recover-guard-cont-hyst-validate"
)
...
Write-Host "[poste14-gui-recover-guard-cont-hyst] focused gui recovery guardrails continuity hysteresis run"
& .\scripts\run-qemu.ps1 -Profile $BuildCfg -TimeoutSeconds $TimeoutSeconds -LogPath $logPath
$qemuExit = $LASTEXITCODE

if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-gui-recover-guard-cont-hyst] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "gui-recover-guard-cont-hyst: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 GUI Recovery Guardrails Continuity Hysteresis Validation: $result",
...
Write-Host "[poste14-gui-recover-guard-cont-hyst] summary -> $summaryPath"
Write-Host "[poste14-gui-recover-guard-cont-hyst] json -> $jsonPath"
