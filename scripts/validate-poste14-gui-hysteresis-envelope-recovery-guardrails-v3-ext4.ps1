#!/usr/bin/env pwsh
# Focused validator for Post-E14 Slice 92: GUI hysteresis envelope recovery guardrails v3 extended baseline (fourth cycle)
[Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSAvoidAssignmentToAutomaticVariable', '', Justification = 'False positive in analyzer; script does not assign to automatic variable $Matches.')]

param(
    [string]$OutPrefix = "build/poste14-guihystenveloperecoverguard3ext4-s92"
)
...
$failPatterns = @(
    "gui-hyst-envelope-recover-guard3-ext4: .*FAIL",
    "page fault",
    "panic"
)
foreach ($pattern in $failPatterns) {
    foreach ($line in @($log)) {
        if ([regex]::IsMatch([string]$line, $pattern)) { $failHits += $line }
    }
}
...
$summary = @()
$summary += "Post-E14 Slice 92: GUI hysteresis envelope recovery guardrails v3 extended baseline (fourth cycle)"
...
$jsonSummary = @{
    slice = 92
    name = "GUI hysteresis envelope recovery guardrails v3 extended baseline (fourth cycle)"
