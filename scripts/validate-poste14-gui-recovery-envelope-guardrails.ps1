param(
    ...
    [string]$OutPrefix = "build/poste14-gui-recover-envelope-guard-validate"
)
...
Write-Host "[poste14-gui-recover-envelope-guard] focused gui recovery envelope guardrails run"
& .\scripts\run-qemu.ps1 -Profile $BuildCfg -TimeoutSeconds $TimeoutSeconds -LogPath $logPath
$qemuExit = $LASTEXITCODE

if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-gui-recover-envelope-guard] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "gui-recover-envelope-guard: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 GUI Recovery Envelope Guardrails Validation: $result",
...
Write-Host "[poste14-gui-recover-envelope-guard] summary -> $summaryPath"
Write-Host "[poste14-gui-recover-envelope-guard] json -> $jsonPath"
