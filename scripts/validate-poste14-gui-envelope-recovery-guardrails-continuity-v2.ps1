param(
    ...
    [string]$OutPrefix = "build/poste14-gui-envelope-recover-guard-cont2-validate"
)
...
Write-Host "[poste14-gui-envelope-recover-guard-cont2] focused gui envelope recovery guardrails continuity v2 run"
& .\scripts\run-qemu.ps1 -Profile $BuildCfg -TimeoutSeconds $TimeoutSeconds -LogPath $logPath
$qemuExit = $LASTEXITCODE

if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-gui-envelope-recover-guard-cont2] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "gui-envelope-recover-guard-cont2: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 GUI Envelope Recovery Guardrails Continuity v2 Validation: $result",
...
Write-Host "[poste14-gui-envelope-recover-guard-cont2] summary -> $summaryPath"
Write-Host "[poste14-gui-envelope-recover-guard-cont2] json -> $jsonPath"
