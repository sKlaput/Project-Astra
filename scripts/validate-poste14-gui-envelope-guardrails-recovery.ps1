param(
    ...
    [string]$OutPrefix = "build/poste14-gui-envelope-guard-recover-validate"
)
...
Write-Host "[poste14-gui-envelope-guard-recover] focused gui envelope guardrails recovery run"
& .\scripts\run-qemu.ps1 -Profile $BuildCfg -TimeoutSeconds $TimeoutSeconds -LogPath $logPath
$qemuExit = $LASTEXITCODE

if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-gui-envelope-guard-recover] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "gui-envelope-guard-recover: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 GUI Envelope Guardrails Recovery Validation: $result",
...
Write-Host "[poste14-gui-envelope-guard-recover] summary -> $summaryPath"
Write-Host "[poste14-gui-envelope-guard-recover] json -> $jsonPath"
