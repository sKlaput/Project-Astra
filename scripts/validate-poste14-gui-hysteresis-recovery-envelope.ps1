param(
    ...
    [string]$OutPrefix = "build/poste14-gui-hyst-recover-envelope-validate"
)
...
Write-Host "[poste14-gui-hyst-recover-envelope] focused gui hysteresis recovery envelope run"
& .\scripts\run-qemu.ps1 -Profile $BuildCfg -TimeoutSeconds $TimeoutSeconds -LogPath $logPath
$qemuExit = $LASTEXITCODE

if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-gui-hyst-recover-envelope] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "gui-hyst-recover-envelope: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 GUI Hysteresis Recovery Envelope Validation: $result",
...
Write-Host "[poste14-gui-hyst-recover-envelope] summary -> $summaryPath"
Write-Host "[poste14-gui-hyst-recover-envelope] json -> $jsonPath"
