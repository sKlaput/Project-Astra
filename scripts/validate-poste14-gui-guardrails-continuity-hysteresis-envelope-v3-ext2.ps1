#!/usr/bin/env pwsh
# Focused validator for Post-E14 Slice 80: GUI guardrails continuity hysteresis envelope v3 extended baseline (second cycle)

param(
    [string]$OutPrefix = "build/poste14-guiguardconthystenvelope3ext2-s80"
)
...
$failPatterns = @(
    "gui-guard-cont-hyst-envelope3-ext2: .*FAIL",
    "page fault",
    "panic"
)
...
$summary = @()
$summary += "Post-E14 Slice 80: GUI guardrails continuity hysteresis envelope v3 extended baseline (second cycle)"
...
$jsonSummary = @{
    slice = 80
    name = "GUI guardrails continuity hysteresis envelope v3 extended baseline (second cycle)"
