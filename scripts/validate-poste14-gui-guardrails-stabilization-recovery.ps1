param(
    ...
    [string]$OutPrefix = "build/poste14-gui-guard-stabilize-recover-validate"
)
...
Write-Host "[poste14-gui-guard-stabilize-recover] focused gui guardrails stabilization recovery run"
& .\scripts\run-qemu.ps1 -Profile $BuildCfg -TimeoutSeconds $TimeoutSeconds -LogPath $logPath
$qemuExit = $LASTEXITCODE

if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-gui-guard-stabilize-recover] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "gui-guard-stabilize-recover: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 GUI Guardrails Stabilization Recovery Validation: $result",
...
Write-Host "[poste14-gui-guard-stabilize-recover] summary -> $summaryPath"
Write-Host "[poste14-gui-guard-stabilize-recover] json -> $jsonPath"
