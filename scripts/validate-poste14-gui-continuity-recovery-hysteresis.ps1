param(
    ...
    [string]$OutPrefix = "build/poste14-gui-cont-recover-hyst-validate"
)
...
Write-Host "[poste14-gui-cont-recover-hyst] focused gui continuity recovery hysteresis run"
& .\scripts\run-qemu.ps1 -Profile $BuildCfg -TimeoutSeconds $TimeoutSeconds -LogPath $logPath
$qemuExit = $LASTEXITCODE

if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-gui-cont-recover-hyst] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "gui-cont-recover-hyst: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 GUI Continuity Recovery Hysteresis Validation: $result",
...
Write-Host "[poste14-gui-cont-recover-hyst] summary -> $summaryPath"
Write-Host "[poste14-gui-cont-recover-hyst] json -> $jsonPath"
