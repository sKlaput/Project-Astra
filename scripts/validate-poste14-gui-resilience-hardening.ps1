param(
    ...
    [string]$OutPrefix = "build/poste14-gui-harden-validate"
)
...
Write-Host "[poste14-gui-harden] focused gui resilience hardening run"
& .\scripts\run-qemu.ps1 -Profile $BuildCfg -TimeoutSeconds $TimeoutSeconds -LogPath $logPath
$qemuExit = $LASTEXITCODE

if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-gui-harden] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "gui-harden: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 GUI Resilience Hardening Validation: $result",
...
Write-Host "[poste14-gui-harden] summary -> $summaryPath"
Write-Host "[poste14-gui-harden] json -> $jsonPath"
