param(
    ...
    [string]$OutPrefix = "build/poste14-gui-stress-validate"
)
...
Write-Host "[poste14-gui-stress] focused gui churn stress run"
& .\scripts\run-qemu.ps1 -Profile $BuildCfg -TimeoutSeconds $TimeoutSeconds -LogPath $logPath
$qemuExit = $LASTEXITCODE

if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-gui-stress] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "gui-stress: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 GUI Churn Stress Validation: $result",
...
Write-Host "[poste14-gui-stress] summary -> $summaryPath"
Write-Host "[poste14-gui-stress] json -> $jsonPath"
