#!/usr/bin/env pwsh
# Focused validator for Post-E14 Slice 86: GUI continuity hysteresis envelope recovery v3 extended baseline (third cycle)

param(
    [string]$OutPrefix = "build/poste14-guiconthystenveloperecover3ext3-s86"
)
...
$failPatterns = @(
    "gui-cont-hyst-envelope-recover3-ext3: .*FAIL",
    "page fault",
    "panic"
)
...
$summary = @()
$summary += "Post-E14 Slice 86: GUI continuity hysteresis envelope recovery v3 extended baseline (third cycle)"
...
$jsonSummary = @{
    slice = 86
    name = "GUI continuity hysteresis envelope recovery v3 extended baseline (third cycle)"
