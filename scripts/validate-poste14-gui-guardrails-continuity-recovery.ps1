param(
    ...
    [string]$OutPrefix = "build/poste14-gui-guard-cont-recover-validate"
)
...
Write-Host "[poste14-gui-guard-cont-recover] focused gui guardrails continuity recovery run"
& .\scripts\run-qemu.ps1 -Profile $BuildCfg -TimeoutSeconds $TimeoutSeconds -LogPath $logPath
$qemuExit = $LASTEXITCODE

if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-gui-guard-cont-recover] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "gui-guard-cont-recover: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 GUI Guardrails Continuity Recovery Validation: $result",
...
Write-Host "[poste14-gui-guard-cont-recover] summary -> $summaryPath"
Write-Host "[poste14-gui-guard-cont-recover] json -> $jsonPath"
