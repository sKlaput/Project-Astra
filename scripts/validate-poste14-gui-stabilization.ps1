param(
    ...
    [string]$OutPrefix = "build/poste14-gui-stabilize-validate"
)
...
Write-Host "[poste14-gui-stabilize] focused gui long-window stabilization run"
& .\scripts\run-qemu.ps1 -Profile $BuildCfg -TimeoutSeconds $TimeoutSeconds -LogPath $logPath
$qemuExit = $LASTEXITCODE

if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-gui-stabilize] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "gui-stabilize: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 GUI Long-Window Stabilization Validation: $result",
...
Write-Host "[poste14-gui-stabilize] summary -> $summaryPath"
Write-Host "[poste14-gui-stabilize] json -> $jsonPath"
