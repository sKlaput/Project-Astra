param(
    ...
    [string]$OutPrefix = "build/poste14-gui-recover-envelope-guard-hyst-validate"
)
...
Write-Host "[poste14-gui-recover-envelope-guard-hyst] focused gui recovery envelope guardrails hysteresis run"
& .\scripts\run-qemu.ps1 -Profile $BuildCfg -TimeoutSeconds $TimeoutSeconds -LogPath $logPath
$qemuExit = $LASTEXITCODE

if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-gui-recover-envelope-guard-hyst] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "gui-recover-envelope-guard-hyst: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 GUI Recovery Envelope Guardrails Hysteresis Validation: $result",
...
Write-Host "[poste14-gui-recover-envelope-guard-hyst] summary -> $summaryPath"
Write-Host "[poste14-gui-recover-envelope-guard-hyst] json -> $jsonPath"
