param(
    ...
    [string]$OutPrefix = "build/poste14-gui-guard-validate"
)
...
Write-Host "[poste14-gui-guard] focused gui recovery guardrails run"
& .\scripts\run-qemu.ps1 -Profile $BuildCfg -TimeoutSeconds $TimeoutSeconds -LogPath $logPath
$qemuExit = $LASTEXITCODE

if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-gui-guard] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "gui-guard: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 GUI Recovery Guardrails Validation: $result",
...
Write-Host "[poste14-gui-guard] summary -> $summaryPath"
Write-Host "[poste14-gui-guard] json -> $jsonPath"
