param(
    ...
    [string]$OutPrefix = "build/poste14-gui-envelope-recover-guard-cont-validate"
)
...
Write-Host "[poste14-gui-envelope-recover-guard-cont] focused gui envelope recovery guardrails continuity run"
& .\scripts\run-qemu.ps1 -Profile $BuildCfg -TimeoutSeconds $TimeoutSeconds -LogPath $logPath
$qemuExit = $LASTEXITCODE

if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-gui-envelope-recover-guard-cont] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "gui-envelope-recover-guard-cont: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 GUI Envelope Recovery Guardrails Continuity Validation: $result",
...
Write-Host "[poste14-gui-envelope-recover-guard-cont] summary -> $summaryPath"
Write-Host "[poste14-gui-envelope-recover-guard-cont] json -> $jsonPath"
