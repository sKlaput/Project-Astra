#!/usr/bin/env pwsh
# Focused validator for Post-E14 Slice 95: GUI guardrails continuity hysteresis envelope v3 extended baseline (fifth cycle)

param(
    [string]$OutPrefix = "build/poste14-guiguardconthystenvelope3ext5-s95"
)
...
$failHits += @($log | Where-Object {
    $_ -like "gui-guard-cont-hyst-envelope3-ext5: *FAIL" -or
...
$summary = @()
$summary += "Post-E14 Slice 95: GUI guardrails continuity hysteresis envelope v3 extended baseline (fifth cycle)"
...
$jsonSummary = @{
    slice = 95
    name = "GUI guardrails continuity hysteresis envelope v3 extended baseline (fifth cycle)"
