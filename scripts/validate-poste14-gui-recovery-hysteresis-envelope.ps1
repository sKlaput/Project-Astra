param(
    ...
    [string]$OutPrefix = "build/poste14-gui-recover-hyst-envelope-validate"
)
...
Write-Host "[poste14-gui-recover-hyst-envelope] focused gui recovery hysteresis envelope run"
& .\scripts\run-qemu.ps1 -Profile $BuildCfg -TimeoutSeconds $TimeoutSeconds -LogPath $logPath
$qemuExit = $LASTEXITCODE

if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-gui-recover-hyst-envelope] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "gui-recover-hyst-envelope: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 GUI Recovery Hysteresis Envelope Validation: $result",
...
Write-Host "[poste14-gui-recover-hyst-envelope] summary -> $summaryPath"
Write-Host "[poste14-gui-recover-hyst-envelope] json -> $jsonPath"
