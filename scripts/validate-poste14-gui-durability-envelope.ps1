param(
    ...
    [string]$OutPrefix = "build/poste14-gui-durenvelope-validate"
)
...
Write-Host "[poste14-gui-dur-envelope] focused gui durability envelope run"
& .\scripts\run-qemu.ps1 -Profile $BuildCfg -TimeoutSeconds $TimeoutSeconds -LogPath $logPath
$qemuExit = $LASTEXITCODE

if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-gui-dur-envelope] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "gui-dur-envelope: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 GUI Durability Envelope Validation: $result",
...
Write-Host "[poste14-gui-dur-envelope] summary -> $summaryPath"
Write-Host "[poste14-gui-dur-envelope] json -> $jsonPath"
