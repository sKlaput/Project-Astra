param(
    ...
    [string]$OutPrefix = "build/poste14-gui-focus-recovery-validate"
)
...
Write-Host "[poste14-gui-recover] focused gui focus recovery run"
& .\scripts\run-qemu.ps1 -Profile $BuildCfg -TimeoutSeconds $TimeoutSeconds -LogPath $logPath
$qemuExit = $LASTEXITCODE

if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-gui-recover] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "gui-recover: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 GUI Focus Recovery Validation: $result",
...
Write-Host "[poste14-gui-recover] summary -> $summaryPath"
Write-Host "[poste14-gui-recover] json -> $jsonPath"
