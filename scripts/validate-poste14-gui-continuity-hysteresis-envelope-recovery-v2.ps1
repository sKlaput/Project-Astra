param(
    ...
    [string]$OutPrefix = "build/poste14-gui-cont-hyst-envelope-recover2-validate"
)
...
Write-Host "[poste14-gui-cont-hyst-envelope-recover2] focused gui continuity hysteresis envelope recovery v2 run"
& .\scripts\run-qemu.ps1 -Profile $BuildCfg -TimeoutSeconds $TimeoutSeconds -LogPath $logPath
$qemuExit = $LASTEXITCODE

if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-gui-cont-hyst-envelope-recover2] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "gui-cont-hyst-envelope-recover2: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 GUI Continuity Hysteresis Envelope Recovery v2 Validation: $result",
...
Write-Host "[poste14-gui-cont-hyst-envelope-recover2] summary -> $summaryPath"
Write-Host "[poste14-gui-cont-hyst-envelope-recover2] json -> $jsonPath"
