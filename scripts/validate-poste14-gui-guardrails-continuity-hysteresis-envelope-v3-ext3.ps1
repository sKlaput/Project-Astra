#!/usr/bin/env pwsh
# Focused validator for Post-E14 Slice 85: GUI guardrails continuity hysteresis envelope v3 extended baseline (third cycle)

param(
    [string]$OutPrefix = "build/poste14-guiguardconthystenvelope3ext3-s85"
)
...
$failPatterns = @(
    "gui-guard-cont-hyst-envelope3-ext3: .*FAIL",
    "page fault",
    "panic"
)
...
$summary = @()
$summary += "Post-E14 Slice 85: GUI guardrails continuity hysteresis envelope v3 extended baseline (third cycle)"
...
$jsonSummary = @{
    slice = 85
    name = "GUI guardrails continuity hysteresis envelope v3 extended baseline (third cycle)"
