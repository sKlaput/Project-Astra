param(
    ...
    [string]$OutPrefix = "build/poste14-gui-durguard-validate"
)
...
Write-Host "[poste14-gui-dur-guard] focused gui durability guardrails run"
& .\scripts\run-qemu.ps1 -Profile $BuildCfg -TimeoutSeconds $TimeoutSeconds -LogPath $logPath
$qemuExit = $LASTEXITCODE

if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-gui-dur-guard] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "gui-dur-guard: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 GUI Durability Guardrails Validation: $result",
...
Write-Host "[poste14-gui-dur-guard] summary -> $summaryPath"
Write-Host "[poste14-gui-dur-guard] json -> $jsonPath"
