param(
    ...
    [string]$OutPrefix = "build/poste14-gui-stabilize-recover-hyst-validate"
)
...
Write-Host "[poste14-gui-stabilize-recover-hyst] focused gui stabilization recovery hysteresis run"
& .\scripts\run-qemu.ps1 -Profile $BuildCfg -TimeoutSeconds $TimeoutSeconds -LogPath $logPath
$qemuExit = $LASTEXITCODE

if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-gui-stabilize-recover-hyst] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "gui-stabilize-recover-hyst: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 GUI Stabilization Recovery Hysteresis Validation: $result",
...
Write-Host "[poste14-gui-stabilize-recover-hyst] summary -> $summaryPath"
Write-Host "[poste14-gui-stabilize-recover-hyst] json -> $jsonPath"
