param(
    ...
    [string]$OutPrefix = "build/poste14-gui-guard-recover-validate"
)
...
Write-Host "[poste14-gui-guard-recover] focused gui guardrails recovery run"
& .\scripts\run-qemu.ps1 -Profile $BuildCfg -TimeoutSeconds $TimeoutSeconds -LogPath $logPath
$qemuExit = $LASTEXITCODE

if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-gui-guard-recover] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "gui-guard-recover: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 GUI Guardrails Recovery Validation: $result",
...
Write-Host "[poste14-gui-guard-recover] summary -> $summaryPath"
Write-Host "[poste14-gui-guard-recover] json -> $jsonPath"
