param(
    ...
    [string]$OutPrefix = "build/poste14-gui-envelope-recover-guard-validate"
)
...
Write-Host "[poste14-gui-envelope-recover-guard] focused gui envelope recovery guardrails run"
& .\scripts\run-qemu.ps1 -Profile $BuildCfg -TimeoutSeconds $TimeoutSeconds -LogPath $logPath
$qemuExit = $LASTEXITCODE

if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-gui-envelope-recover-guard] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "gui-envelope-recover-guard: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 GUI Envelope Recovery Guardrails Validation: $result",
...
Write-Host "[poste14-gui-envelope-recover-guard] summary -> $summaryPath"
Write-Host "[poste14-gui-envelope-recover-guard] json -> $jsonPath"
