param(
    ...
    [string]$OutPrefix = "build/poste14-gui-guard-cont-hyst-envelope2-validate"
)
...
Write-Host "[poste14-gui-guard-cont-hyst-envelope2] focused gui guardrails continuity hysteresis envelope v2 run"
& .\scripts\run-qemu.ps1 -Profile $BuildCfg -TimeoutSeconds $TimeoutSeconds -LogPath $logPath
$qemuExit = $LASTEXITCODE

if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-gui-guard-cont-hyst-envelope2] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "gui-guard-cont-hyst-envelope2: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 GUI Guardrails Continuity Hysteresis Envelope v2 Validation: $result",
...
Write-Host "[poste14-gui-guard-cont-hyst-envelope2] summary -> $summaryPath"
Write-Host "[poste14-gui-guard-cont-hyst-envelope2] json -> $jsonPath"
