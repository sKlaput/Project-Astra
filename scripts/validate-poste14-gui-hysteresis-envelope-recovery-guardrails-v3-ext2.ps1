#!/usr/bin/env pwsh
# Focused validator for Post-E14 Slice 82: GUI hysteresis envelope recovery guardrails v3 extended baseline (second cycle)

param(
    [string]$OutPrefix = "build/poste14-guihystenveloperecoverguard3ext2-s82"
)
...
$failPatterns = @(
    "gui-hyst-envelope-recover-guard3-ext2: .*FAIL",
    "page fault",
    "panic"
)
...
$summary = @()
$summary += "Post-E14 Slice 82: GUI hysteresis envelope recovery guardrails v3 extended baseline (second cycle)"
...
$jsonSummary = @{
    slice = 82
    name = "GUI hysteresis envelope recovery guardrails v3 extended baseline (second cycle)"
