#!/usr/bin/env pwsh
# Focused validator for Post-E14 Slice 81: GUI continuity hysteresis envelope recovery v3 extended baseline (second cycle)

param(
    [string]$OutPrefix = "build/poste14-guiconthystenveloperecover3ext2-s81"
)
...
$failPatterns = @(
    "gui-cont-hyst-envelope-recover3-ext2: .*FAIL",
    "page fault",
    "panic"
)
...
$summary = @()
$summary += "Post-E14 Slice 81: GUI continuity hysteresis envelope recovery v3 extended baseline (second cycle)"
...
$jsonSummary = @{
    slice = 81
    name = "GUI continuity hysteresis envelope recovery v3 extended baseline (second cycle)"
