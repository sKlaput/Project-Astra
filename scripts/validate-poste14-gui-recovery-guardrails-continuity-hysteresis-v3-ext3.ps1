#!/usr/bin/env pwsh
# Focused validator for Post-E14 Slice 84: GUI recovery guardrails continuity hysteresis v3 extended baseline (third cycle)

param(
    [string]$OutPrefix = "build/poste14-guirecoverguardconthyst3ext3-s84"
)
...
$failPatterns = @(
    "gui-recover-guard-cont-hyst3-ext3: .*FAIL",
    "page fault",
    "panic"
)
...
$summary = @()
$summary += "Post-E14 Slice 84: GUI recovery guardrails continuity hysteresis v3 extended baseline (third cycle)"
...
$jsonSummary = @{
    slice = 84
    name = "GUI recovery guardrails continuity hysteresis v3 extended baseline (third cycle)"
