param(
    ...
    [string]$OutPrefix = "build/poste14-gui-cooldown-validate"
)
...
Write-Host "[poste14-gui-cooldown] focused gui escalation cooldown run"
& .\scripts\run-qemu.ps1 -Profile $BuildCfg -TimeoutSeconds $TimeoutSeconds -LogPath $logPath
$qemuExit = $LASTEXITCODE

if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-gui-cooldown] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "gui-cooldown: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 GUI Escalation Cooldown Validation: $result",
...
Write-Host "[poste14-gui-cooldown] summary -> $summaryPath"
Write-Host "[poste14-gui-cooldown] json -> $jsonPath"
