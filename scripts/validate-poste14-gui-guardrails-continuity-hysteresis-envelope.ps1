param(
    ...
    [string]$OutPrefix = "build/poste14-gui-guard-cont-hyst-envelope-validate"
)
...
Write-Host "[poste14-gui-guard-cont-hyst-envelope] focused gui guardrails continuity hysteresis envelope run"
& .\scripts\run-qemu.ps1 -Profile $BuildCfg -TimeoutSeconds $TimeoutSeconds -LogPath $logPath
$qemuExit = $LASTEXITCODE

if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-gui-guard-cont-hyst-envelope] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "gui-guard-cont-hyst-envelope: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 GUI Guardrails Continuity Hysteresis Envelope Validation: $result",
...
Write-Host "[poste14-gui-guard-cont-hyst-envelope] summary -> $summaryPath"
Write-Host "[poste14-gui-guard-cont-hyst-envelope] json -> $jsonPath"
