param(
    ...
    [string]$OutPrefix = "build/poste14-gui-cont-hyst-envelope-recover-validate"
)
...
Write-Host "[poste14-gui-cont-hyst-envelope-recover] focused gui continuity hysteresis envelope recovery run"
& .\scripts\run-qemu.ps1 -Profile $BuildCfg -TimeoutSeconds $TimeoutSeconds -LogPath $logPath
$qemuExit = $LASTEXITCODE

if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-gui-cont-hyst-envelope-recover] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "gui-cont-hyst-envelope-recover: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 GUI Continuity Hysteresis Envelope Recovery Validation: $result",
...
Write-Host "[poste14-gui-cont-hyst-envelope-recover] summary -> $summaryPath"
Write-Host "[poste14-gui-cont-hyst-envelope-recover] json -> $jsonPath"
