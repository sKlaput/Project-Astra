param(
    ...
    [string]$OutPrefix = "build/poste14-gui-durable-validate"
)
...
Write-Host "[poste14-gui-durable] focused gui envelope durability run"
& .\scripts\run-qemu.ps1 -Profile $BuildCfg -TimeoutSeconds $TimeoutSeconds -LogPath $logPath
$qemuExit = $LASTEXITCODE

if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-gui-durable] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "gui-durable: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 GUI Envelope Durability Validation: $result",
...
Write-Host "[poste14-gui-durable] summary -> $summaryPath"
Write-Host "[poste14-gui-durable] json -> $jsonPath"
