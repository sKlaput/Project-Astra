param(
    ...
    [string]$OutPrefix = "build/poste14-gui-guard-recover-cont-validate"
)
...
Write-Host "[poste14-gui-guard-recover-cont] focused gui guardrails recovery continuity run"
& .\scripts\run-qemu.ps1 -Profile $BuildCfg -TimeoutSeconds $TimeoutSeconds -LogPath $logPath
$qemuExit = $LASTEXITCODE

if (-not (Test-Path $logPath)) {
    Write-Host "[poste14-gui-guard-recover-cont] FAIL: missing log $logPath"
    exit 1
}
...
$failHits = @(
    Select-String -Path $logPath -Pattern "gui-guard-recover-cont: .*FAIL|page fault|panic" -CaseSensitive:$false |
...
$lines = @(
    "Post-E14 GUI Guardrails Recovery Continuity Validation: $result",
...
Write-Host "[poste14-gui-guard-recover-cont] summary -> $summaryPath"
Write-Host "[poste14-gui-guard-recover-cont] json -> $jsonPath"
